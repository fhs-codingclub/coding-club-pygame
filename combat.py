...
class BattleSystem:
    ...
    def draw_menu(self):
        ...
        pygame.draw.rect(
            screen,
            wood_base,
            (0, panel_top, SCREEN_WIDTH, panel_height)
        )
        pygame.draw.rect(
            screen,
            wood_dark,
        ...
        for i, option in enumerate(self.menu_options):
            ...
            color = wood_light if (
                self.state == "menu" and i == self.selected_option
            ) else wood_base
            ...
            if self.item_options:
                for i, item in enumerate(self.item_options):
                    ...
                    text = small_font.render(
                        f"{item} x{self.player.items[item]}",
                        True,
                        WHITE
                    )
        ...
        if self.state == "menu":
            ...
            if event.key == pygame.K_UP:
                ...
                self.selected_item = min(
                    len(self.item_options) - 1,
                    self.selected_item + 1
                )
            elif event.key in (pygame.K_RETURN, pygame.K_z):
                if self.item_options:
                    self.use_item(
                        self.item_options[self.selected_item]
                    )
